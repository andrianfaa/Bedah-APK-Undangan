    ...
    public static boolean measure(int level, ConstraintWidget widget, BasicMeasure.Measurer measurer, BasicMeasure.Measure measure, int measureStrategy) {
        ...
        if (z3) {
            if (widget.mResolvedMatchConstraintDefault[0] == 4) {
                ...
                measure.horizontalBehavior = ConstraintWidget.DimensionBehaviour.FIXED;
                measure.horizontalDimension = (int) (widget.getDimensionRatio() * ((float) i2));
            }
        }
        if (z4) {
            if (widget.mResolvedMatchConstraintDefault[1] == 4) {
                ...
                if (widget.getDimensionRatioSide() == -1) {
                    measure.verticalDimension = (int) (((float) i) / widget.getDimensionRatio());
                } else {
                    measure.verticalDimension = (int) (widget.getDimensionRatio() * ((float) i));
                }
            }
        }
        measurer.measure(widget, measure);
        ...
        if (LinearSystem.USE_DEPENDENCY_ORDERING) {
            ...
            for (int i5 = 0; i5 < size; i5++) {
                ...
                if (constraintWidget6 instanceof ConstraintWidgetContainer) {
                    ...
                    constraintWidget6.addToSolver(system, optimizeFor);
                    if (dimensionBehaviour == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                    ...
                    Optimizer.checkMatchParent(this, system, constraintWidget6);
