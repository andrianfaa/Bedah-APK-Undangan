...
public final class BitmapCompat {
    ...
    static class Api27Impl {
        ...
        static Bitmap copyBitmapIfHardware(Bitmap bm) {
            ...
            if (Build.VERSION.SDK_INT >= 31) {
                config = Api31Impl.getHardwareBitmapConfig(bm);
            }
            return bm.copy(config, true);
        }
    ...
    static class Api31Impl {
        ...
        static Bitmap.Config getHardwareBitmapConfig(Bitmap bm) {
            return bm.getHardwareBuffer().getFormat() == 22 ? Bitmap.Config.RGBA_F16 : Bitmap.Config.ARGB_8888;
        }
    }
    ...
    public static int sizeAtStep(int srcSize, int dstSize, int step, int totalSteps) {
        return step == 0 ? dstSize : step > 0 ? (1 << (totalSteps - step)) * srcSize : dstSize << ((-step) - 1);
    }
}
