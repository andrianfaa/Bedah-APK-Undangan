...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long currentTime) {
            ...
            if (j2 < 0 || currentTime < j2) {
                return AutoScrollHelper.constrain(((float) (currentTime - j)) / ((float) this.mRampUpDuration), 0.0f, 1.0f) * 0.5f;
            }
    ...
    private float constrainEdgeValue(float current, float leading) {
        ...
        switch (i) {
            ...
            case 1:
                if (current < leading) {
                    return current >= 0.0f ? 1.0f - (current / leading) : (!this.mAnimating || i != 1) ? 0.0f : 1.0f;
                }
                break;
            case 2:
                if (current < 0.0f) {
    ...
    private float getEdgeValue(float relativeValue, float size, float maxValue, float current) {
        ...
        if (constrainEdgeValue < 0.0f) {
            f = -this.mEdgeInterpolator.getInterpolation(-constrainEdgeValue);
