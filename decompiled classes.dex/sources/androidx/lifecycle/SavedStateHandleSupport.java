...
public final class SavedStateHandleSupport {
    ...
    public static final SavedStateHandle createSavedStateHandle(CreationExtras $this$createSavedStateHandle) {
        ...
        if (savedStateRegistryOwner != null) {
            ...
            if (viewModelStoreOwner != null) {
                Bundle bundle = (Bundle) $this$createSavedStateHandle.get(DEFAULT_ARGS_KEY);
                String str = (String) $this$createSavedStateHandle.get(ViewModelProvider.NewInstanceFactory.VIEW_MODEL_KEY);
                if (str != null) {
    ...
    private static final SavedStateHandle createSavedStateHandle(SavedStateRegistryOwner savedStateRegistryOwner, ViewModelStoreOwner viewModelStoreOwner, String key, Bundle defaultArgs) {
        ...
        SavedStateHandle savedStateHandle = savedStateHandlesVM.getHandles().get(key);
        if (savedStateHandle != null) {
            return savedStateHandle;
        }
        SavedStateHandle createHandle = SavedStateHandle.Companion.createHandle(savedStateHandlesProvider.consumeRestoredStateForKey(key), defaultArgs);
        savedStateHandlesVM.getHandles().put(key, createHandle);
        return createHandle;
    }
    ...
    public static final <T extends SavedStateRegistryOwner & ViewModelStoreOwner> void enableSavedStateHandles(T $this$enableSavedStateHandles) {
        ...
        if (!(currentState == Lifecycle.State.INITIALIZED || currentState == Lifecycle.State.CREATED)) {
            ...
            SavedStateHandlesProvider savedStateHandlesProvider = new SavedStateHandlesProvider($this$enableSavedStateHandles.getSavedStateRegistry(), (ViewModelStoreOwner) $this$enableSavedStateHandles);
            $this$enableSavedStateHandles.getSavedStateRegistry().registerSavedStateProvider(SAVED_STATE_KEY, savedStateHandlesProvider);
            $this$enableSavedStateHandles.getLifecycle().addObserver(new SavedStateHandleAttacher(savedStateHandlesProvider));
        }
    }
    ...
    public static final SavedStateHandlesVM getSavedStateHandlesVM(ViewModelStoreOwner $this$savedStateHandlesVM) {
        ...
        return (SavedStateHandlesVM) new ViewModelProvider($this$savedStateHandlesVM, initializerViewModelFactoryBuilder.build()).get(VIEWMODEL_KEY, SavedStateHandlesVM.class);
    }
}
