...
class NumbersKt__FloorDivModKt extends NumbersKt__BigIntegersKt {
    ...
    private static final long floorDiv(long $this$floorDiv, long other) {
        long j = $this$floorDiv / other;
        return (($this$floorDiv ^ other) >= 0 || j * other == $this$floorDiv) ? j : j - 1;
    }
    ...
    private static final float mod(float $this$mod, float other) {
        float f = $this$mod % other;
        boolean z = true;
        if (!(f == 0.0f)) {
            if (Math.signum(f) != Math.signum(other)) {
                z = false;
            }
            if (!z) {
                return f + other;
