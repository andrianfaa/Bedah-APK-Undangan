...
class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double acosh(double x) {
        ...
        double sqrt = Math.sqrt(x - d);
        double d2 = sqrt;
        if (sqrt >= Constants.taylor_2_bound) {
            d2 -= ((sqrt * sqrt) * sqrt) / ((double) 12);
        }
        return Math.sqrt(2.0d) * d2;
    }
    ...
    public static final double asinh(double x) {
        if (x >= Constants.taylor_n_bound) {
            ...
            double d2 = x;
            return Math.abs(x) >= Constants.taylor_2_bound ? d2 - (((x * x) * x) / ((double) 6)) : d2;
        }
    }
    ...
    public static final double atanh(double x) {
        if (Math.abs(x) < Constants.taylor_n_bound) {
            double d = x;
            return Math.abs(x) > Constants.taylor_2_bound ? d + (((x * x) * x) / ((double) 3)) : d;
        }
        double d2 = (double) 1;
        return Math.log((d2 + x) / (d2 - x)) / ((double) 2);
    }
