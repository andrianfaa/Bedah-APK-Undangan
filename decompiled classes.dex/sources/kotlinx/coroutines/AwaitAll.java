...
final class AwaitAll<T> {
    ...
    private final class AwaitAllNode extends JobNode {
        ...
        public void invoke(Throwable cause) {
            ...
            if (AwaitAll.notCompletedCount$FU.decrementAndGet(AwaitAll.this) == 0) {
                ...
                continuation2.resumeWith(Result.m36constructorimpl((List) arrayList));
            }
        }
    ...
    public final Object await(Continuation<? super List<? extends T>> $completion) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            ...
            AwaitAllNode awaitAllNode2 = awaitAllNode;
            awaitAllNode2.setHandle(deferred.invokeOnCompletion(awaitAllNode2));
            Unit unit = Unit.INSTANCE;
            awaitAllNodeArr[i2] = awaitAllNode;
        }
        AwaitAllNode[] awaitAllNodeArr2 = awaitAllNodeArr;
        DisposeHandlersOnCancel disposeHandlersOnCancel = new DisposeHandlersOnCancel(awaitAllNodeArr2);
        AwaitAllNode[] awaitAllNodeArr3 = awaitAllNodeArr2;
        int length2 = awaitAllNodeArr3.length;
        while (i < length2) {
            AwaitAllNode awaitAllNode3 = awaitAllNodeArr3[i];
            i++;
            awaitAllNode3.setDisposer(disposeHandlersOnCancel);
        }
