...
public final class DeflaterSink implements Sink {
    ...
    private final void deflate(boolean syncFlush) {
        ...
        while (true) {
            ...
            int deflate = syncFlush ? this.deflater.deflate(writableSegment$okio.data, writableSegment$okio.limit, 8192 - writableSegment$okio.limit, 2) : this.deflater.deflate(writableSegment$okio.data, writableSegment$okio.limit, 8192 - writableSegment$okio.limit);
            if (deflate > 0) {
                writableSegment$okio.limit += deflate;
                buffer.setSize$okio(buffer.size() + ((long) deflate));
    ...
    public void write(Buffer source, long byteCount) throws IOException {
        ...
        while (j > 0) {
            ...
            deflate(false);
            source.setSize$okio(source.size() - ((long) min));
            segment.pos += min;
